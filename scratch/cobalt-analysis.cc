...
NS_LOG_COMPONENT_DEFINE("SimpleCobaltExample");
...
int main(int argc, char *argv[])
{
  ...
  // Enable logging
  LogComponentEnable("SimpleCobaltExample", LOG_LEVEL_INFO);

  // Create a simple topology: n0 -- r1 -- r2 -- n3
  // Where r1-r2 is the bottleneck link with CoBALT
  NodeContainer nodes;
  nodes.Create(4);
  ...
  // Setup CoBALT queue disc on the bottleneck interface
  ...
  NS_LOG_INFO("Running simulation with CoBALT for " << simTime << " seconds");
  ...
  std::cout << "CoBALT Queue Statistics:" << std::endl;
